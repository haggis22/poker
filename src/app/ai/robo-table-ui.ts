...
export class RoboTableUI implements MessageHandler, CommandBroadcaster {
    ...
    private tableSnapshotAction(action: TableSnapshotAction): void {
        ...
        const requestedSeatID: number = this.user.id == 7 ? 3 : null;
        this.broadcastCommand(new RequestSeatCommand(this.table.id, requestedSeatID));
    ...
    private updateBetsAction(action: UpdateBetsAction): void {
        ...
        let betsString = Object.keys(this.table.betStatus.bets).map(seatIndexString => {

            let seatIndex = parseInt(seatIndexString, 10);
